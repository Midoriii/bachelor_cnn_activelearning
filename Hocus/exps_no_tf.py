...
def getArgs():
    ...
    # How many epochs to training
    desired_epochs = np.int(args[4])
    # By how much % to increase the available set size
    percentage_increase = np.int(args[5])
    # How precise oracle is in %
    oracle_correct = np.int(args[6])
    # Use pseudolabels?
    pseudo_labels = args[7]
    # Ouput file path
    out = args[8]

    return dataset, mdl, mthd, percentage_limit, desired_epochs, percentage_increase, oracle_correct, pseudo_labels, out
...
def actLearning(model, percentage_limit, desired_epochs, oracle_correct, pseudo_labels, qs, K, actTrnData, trnData, trnLabels, actTrnLabels, tstData, tstLabels, actPoolData, actPoolLabels):
    # Initial error
    ...
    while max(labeled) < percentage_limit:
        ...
        if(pseudo_labels == 'yes'):
            ...
            model.fit(
                ...
                batch_size=48, epochs=desired_epochs, verbose=1,
...
def passLearning(model, percentage_limit, desired_epochs, percentage_increase, trnData, trnLabels, tstData, tstLabels):
    # Initial error
    err = getError(model, tstData, tstLabels)
    # Initial labeled %
    labeled = [10]
...
def main():
    ...
    if mthd == "base" or mthd == "all":
        print("\nRunning base\n")
        baseErr, labeled = passLearning(model, percentage_limit, desired_epochs, percentage_increase, trnData, trnLabels, tstData, tstLabels)
    ...
    if mthd == "rs" or mthd == "all":
        ...
        rsErr, labeled = actLearning(model, percentage_limit, desired_epochs, oracle_correct, pseudo_labels, qs, K, actTrnData, trnData, trnLabels, actTrnLabels, tstData, tstLabels, actPoolData, actPoolLabels)
        # Load inital weights for other methods to use
        model.load_weights('tmpweights.h5')
    ...
    if mthd == "lc" or mthd == "all":
        ...
        lcErr, labeled = actLearning(model, percentage_limit, desired_epochs, oracle_correct, pseudo_labels, qs, K, actTrnData, trnData, trnLabels, actTrnLabels, tstData, tstLabels, actPoolData, actPoolLabels)
        # Load inital weights for other methods to use
        model.load_weights('tmpweights.h5')
    ...
    if mthd == "sm" or mthd == "all":
        ...
        smErr, labeled = actLearning(model, percentage_limit, desired_epochs, oracle_correct, pseudo_labels, qs, K, actTrnData, trnData, trnLabels, actTrnLabels, tstData, tstLabels, actPoolData, actPoolLabels)
        # Load inital weights for other methods to use
        model.load_weights('tmpweights.h5')
    ...
    if mthd == "ent" or mthd == "all":
        ...
        entErr, labeled = actLearning(model, percentage_limit, desired_epochs, oracle_correct, pseudo_labels, qs, K, actTrnData, trnData, trnLabels, actTrnLabels, tstData, tstLabels, actPoolData, actPoolLabels)
        # Load inital weights for other methods to use
        model.load_weights('tmpweights.h5')
    ...
    if mthd == "kcg" or mthd == "all":
        ...
        kcgErr, labeled = actLearning(model, percentage_limit, desired_epochs, oracle_correct, pseudo_labels, qs, K, actTrnData, trnData, trnLabels, actTrnLabels, tstData, tstLabels, actPoolData, actPoolLabels)
        # Load inital weights for other methods to use
        model.load_weights('tmpweights.h5')
